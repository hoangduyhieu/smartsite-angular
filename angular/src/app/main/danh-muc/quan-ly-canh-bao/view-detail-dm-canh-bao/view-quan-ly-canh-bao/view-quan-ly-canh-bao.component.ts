...
import {
  ...
  SyncDuLieuCamBienInputDto,
  SyncDuLieuCamBienOutputDto
...
@Component({
  selector: 'app-view-quan-ly-canh-bao',
  templateUrl: './view-quan-ly-canh-bao.component.html',
  styleUrls: ['./view-quan-ly-canh-bao.component.scss']
})
export class ViewQuanLyCanhBaoComponent extends AppComponentBase implements OnInit {

  @Output() onSave = new EventEmitter<any>();
  form: FormGroup;
  saving = false;
  isEdit = false;
  event: DMCanhBaoOutput;
  isView: boolean;
  mucDo: any;
  view = '';
  ...
  ngOnInit() {
    ...
    this.isEdit = true;
    // Sửa
    this.input.tramId = this.event.idTram;
    this.input.limit = 1;
  ...
  getListCB() {
    this._duLieuCamBienServiceProxy.getListDuLieuCBForSelectBox(this.event.idTram).subscribe(rs => {
      this.listDuLieu = rs;
    });
  }
  ...
  private _setValueForEdit() {
    ...
    this.form.controls.txtTenTram.setValue(this.event.tenTram);
