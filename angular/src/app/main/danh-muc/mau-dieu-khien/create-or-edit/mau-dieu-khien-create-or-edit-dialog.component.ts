...
import {
    ...
    CreateOrEditMauDieuKhien, MauDieuKhienThuocTinh, KieuDuLieuLookupTable
...
@Component({
    selector: 'app-mau-dieu-khien-create-or-edit-dialog',
    templateUrl: './mau-dieu-khien-create-or-edit-dialog.component.html',
    styleUrls: ['./mau-dieu-khien-create-or-edit-dialog.component.scss']
})
export class MauDieuKhienCreateOrEditDialogComponent extends AppComponentBase implements OnInit {
    ...
    id: number;
    isView: boolean;
    input: CreateOrEditMauDieuKhien = new CreateOrEditMauDieuKhien();
    ...
    ngOnInit() {
        ...
        this._mauThuocTinhService.getAllLookupTableThuocTinh().subscribe(rs => {
            ...
            if (!this.id) {
                // Thêm mới
                this.isEdit = false;
                this.input = new CreateOrEditMauDieuKhien();
            } else {
                this.isEdit = true;
                // Sửa
                this._mauDieuKhienService.getForEdit(this.id, this.isView).subscribe(item => {
                    this.input = item;
    ...
    save(): void {
        if (CommonComponent.getControlErr(this.form) === '') {
            ...
            this._mauDieuKhienService.createOrEdit(this.input).pipe(
                finalize(() => {
                    this.saving = false;
                })
            ).subscribe((result) => {
    ...
    private _getValueForSave() {
        ...
        this.form.controls.skills.value.forEach(item => {
            ...
            this.input.mauDieuKhienThuocTinh.push(inputCollection);
        });
    }
    ...
    private _setValueForEdit() {
        ...
        for (const item of this.input.mauDieuKhienThuocTinh) {
