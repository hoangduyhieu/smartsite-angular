...
import {
    ...
    DMTramServiceProxy, KieuDuLieuLookupTable, MauThuocTinh, MauThuocTinhDto, ThuocTinhCanhBaoTram, TramCanhBaoThuocTinh, TramDto
...
@Component({
    ...
    templateUrl: './create-or-edit-nhieu-tram-canh-bao.component.html',
    animations: [appModuleAnimation()],
})
export class CreateOrEditNhieuMauCanhBaoTramComponent extends AppComponentBase implements OnInit {

    @Output() onSave = new EventEmitter<any>();
    input: ThuocTinhCanhBaoTram = new ThuocTinhCanhBaoTram();
    form: FormGroup;
    saving = false;
    isEdit = false;
    id: number;
    ...
    ngOnInit(): void {
        ...
        this._mauThuocTinhService.getAllLookupTableThuocTinh().subscribe(rs => {
            ...
            this.input = new ThuocTinhCanhBaoTram();
            if (!this.record) {
                // Thêm mới
                this.isEdit = false;
            } else {
    ...
    save(): void {
        if (CommonComponent.getControlErr(this.form) === '') {
            ...
            if (!this.record) {
                ...
                const check1 = this.listId.filter((e, i) => i !== this.idRecord).includes(this.input.mauThuocTinhId);
                if (check1) {
                    this.showWarningPopUpMessage(this.l('cht_thuoctinhdatontai'));
                    this.saving = false;
                } else {
